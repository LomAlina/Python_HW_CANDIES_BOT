    ...
    if (message.text).isdigit():
        if 0 < int(message.text) < 29:
            ...
            if model.check_win(total):
                await bot.send_message(player.id, f'Победил(а) {player.first_name}')
                return
            model.set_total_candies(total)
            await enemy_turn(player)

        else:
            await bot.send_message(message.from_user.id, 'А не многовато ли взял(а)?')
    else:
...
async def enemy_turn(player):
    ...
    await bot.send_message(player.id, f'Бот взял {enemy_take} конфет, '
                                      f'и на столе осталось {total}')
    if model.check_win(total):
        await bot.send_message(player.id, f'{player.first_name} ты проиграл(а),'
